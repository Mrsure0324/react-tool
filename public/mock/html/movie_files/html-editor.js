(function () {
  ...
  function isOriginAllowed(origin) {
    ...
    if (EDIT_SLIDE_CONSTANT.ParentOriginList.includes(origin)) {
      console.log('isOriginAllowed正常', origin);
      return true
    }
...
`;
  ...
  function handlerElementMouseover(event) {
    if (!isCanEditSlide) {
      return
    }
  ...
  function handlerElementClick(event) {
    if (!isCanEditSlide) {
      return
    }
    const target = event.target;
  ...
  function handlerMessage(event) {
    if (!isOriginAllowed(event.origin)) {
      console.log('isOriginAllowed异常', event.origin);
      return
    }

    
    console.log('handlerMessage触发！', event.data.type === POST_MESSAGE_EVENT_TYPE.SlideEditMode);
    if (event.data.type === POST_MESSAGE_EVENT_TYPE.SlideEditMode) {
      isCanEditSlide = event.data.payload.isCanEditSlide;
      console.log('handlerMessage触发！', isCanEditSlide);
      if (!isCanEditSlide) {
        removeSelectedEleAttributes();
      }
    }
  ...
  function slideEditPostMessage() {
    document.addEventListener('mouseover', handlerElementMouseover);
    document.addEventListener('click', handlerElementClick);
    window.addEventListener('message', handlerMessage);
  }

  function slideSelectedEdit() {
    slideSelectedStyle();
    slideEditPostMessage();
  }
